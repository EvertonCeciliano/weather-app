    ...
    if (navigator.geolocation) {
      navigator.geolocation.getCurrentPosition(
        ...
        (error) => {
          ...
          setError(
            "Não foi possível obter sua localização. Por favor, digite o nome da sua cidade."
          );
          fetchWeather();
        }
      );
      ...
      setError(
        "Seu navegador não suporta geolocalização. Por favor, digite o nome da sua cidade."
      );
      fetchWeather();
    }
  };
    ...
    try {
      ...
      if (axios.isAxiosError(err)) {
        if (err.response?.status === 401) {
          setError(
            "Ops! Parece que estamos com um problema técnico. Por favor, tente novamente mais tarde."
          );
        } else if (err.response?.status === 404) {
          setError(
            "Não encontramos essa cidade. Verifique se o nome está correto e tente novamente."
          );
        } else {
          setError(
            "Algo deu errado ao buscar o clima. Por favor, tente novamente."
          );
        }
      } else {
        setError(
          "Não foi possível buscar o clima no momento. Por favor, tente novamente."
        );
      }
      setWeather(null);
      setForecast(null);
